...
(async()=>{
 ...
 results.push({transferValidation:await evaluate(`(()=>{document.querySelector('#continue').click();const f=document.querySelector('#form');return {invalid:!f.checkValidity(),required:[...f.querySelectorAll(':invalid')].map(e=>e.name||e.id).sort()}})()`)});
 ...
 await navigate('/compare/');for(let attempt=0;attempt<30;attempt++){if(await evaluate("Boolean(document.querySelector('#player-access')&&!document.querySelector('#player-access').hidden)"))break;await sleep(100)}require('assert/strict').equal(await evaluate("document.querySelector('#player-access').hidden"),false);await evaluate(`document.querySelector('#visitor-id').value='24649596';document.querySelector('#check-player').click()`);await sleep(800);await evaluate(`document.querySelector('#access-continue').click()`);await evaluate(`document.querySelector('#opponent').value='82';document.querySelector('#compare').click()`);await sleep(800);
 results.push({comparison:await evaluate(`(()=>{const cell=document.querySelector('#strength th');if(!cell)throw Error('Approved comparison did not render');const before=cell.textContent;KSPreferences.setLanguage('es');return {before,after:cell?.textContent,sameCell:cell===document.querySelector('#strength th'),value:document.querySelector('#opponent').value}})()`)});
 ...
 const adminMessages=[];
 const populatedAdmin={'/admin/users/':'[data-save]','/admin/transfers/':'[data-status-id]','/admin/kings-buffs/':'[data-status]','/admin/audit/':'#events .event','/admin/usage/':'#recent tbody tr'};
 for(const route of ['/admin/users/','/admin/transfers/','/admin/kings-buffs/','/admin/audit/','/admin/usage/','/admin/player-search/']){
   ...
   if(populatedAdmin[route]){
     let loaded=false;
     for(let attempt=0;attempt<30;attempt++){loaded=await evaluate(`Boolean(document.querySelector(${JSON.stringify(populatedAdmin[route])}))`);if(loaded)break;await sleep(100)}
     require('assert/strict').equal(loaded,true,route+': populated admin fixture');
   }
 ...
 assert.deepEqual(get('transferValidation'),{invalid:true,required:['consent','passes','reason']});
