...
(async()=>{
 ...
 signedIn=true;await navigate('/admin/');await sleep(900);
 ...
 await evaluate('history.forward()');await sleep(600);results.push({forward:await evaluate(`({path:location.pathname,lang:document.documentElement.lang,selector:document.querySelector('.site-language-select').value})`)});
 results.push({errors,requests});
 const allLayouts=[];
 for(const route of routes){await navigate(route);for(const theme of ['light','dark'])for(const width of [1280,375])for(const zoom of [1,2]){
 await call('Emulation.setDeviceMetricsOverride',{width,height:900,deviceScaleFactor:1,mobile:false});
 allLayouts.push({route,theme,width,zoom,...await evaluate(`(()=>{document.documentElement.style.zoom='${zoom}';document.documentElement.dataset.theme='${theme}';KSPreferences.setLanguage('fr');const c=document.querySelector('.site-controls').getBoundingClientRect(),n=document.querySelector('.site-header-nav-link').getBoundingClientRect();return {below:n.top>=c.bottom,right:n.right<=innerWidth+1}})()`)});
 }}results.push({allLayouts});
 ...
 assert.equal(allLayouts.filter(x=>!x.below||!x.right).length,0,'Header layout');
 ...
 console.log(JSON.stringify({routes:routes.length,errors:errors.length,layoutFailures:allLayouts.filter(x=>!x.below||!x.right),checks:results.filter(x=>!x.route&&!x.layouts&&!x.adminLayouts&&!x.allLayouts&&!x.errors)}));ws.close();
